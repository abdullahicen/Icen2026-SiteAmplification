...
def compute_site_amplification(Vs30, PGA_ref, model: str, periods,
    ...
    PGA_ref : float
        Rock/reference PSA (e.g. PGA) at rock (same units as used in model).
    model : str
    ...
    for coeffs in coeff_list:
        ...
        f_nonlinear = c2 * nonlinear_shape * np.log((PGA_ref + c4) / c4)

        # Total ln amplification and amplification factor
        ln_amp = f_linear + f_nonlinear
        Amp = np.exp(ln_amp)

        results.append(Amp)

    # Shape: (n_periods, n_Vs30)
