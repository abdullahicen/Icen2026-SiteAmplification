...
Reads:
    ...
    - PSAr.txt     : rock PSA values for each period (same length/order as Periods.txt)
...
Writes:
    - Output.xlsx  : same input columns + amplification for each period
...
PSAR_FILE    = os.path.join(BASE_DIR, "PSAr.txt")
...
def load_psar(path):
    """Read PSAr(T) from PSAr.txt as floats (one value per line)."""
    psar_vals = []
    with open(path, "r") as f:
        for line in f:
            s = line.strip()
...
def main():
    # 1) Load periods and PSAr(T)
    periods = load_periods(PERIODS_FILE)
    psar_list = load_psar(PSAR_FILE)

    if len(psar_list) != len(periods):
        raise ValueError(
            f"PSAr.txt length ({len(psar_list)}) does not match "
            f"Periods.txt length ({len(periods)})."
        )
    ...
    required_cols = ["Station", "Vs30", "Latitude", "Longitude"]
